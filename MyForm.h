	...
	private: System::Void button1_Click(System::Object^  sender, System::EventArgs^  e) {
		if (openFileDialog1->ShowDialog() == System::Windows::Forms::DialogResult::OK)
		{
			...
			while (i <= slot) {
				...
				//define array k_cheap and other
				for (int i2 = 0; i2 < 100; i2++){
					k_cheap[i2] = 0;
				}

				int recover = System::Convert::ToInt32(sorted_appliance[i, 3]);

				//searh k_cheap
				for (int n = 1; n <= prog_count; n++){
					id = 0;
					sorted_appliance[i, 3] = System::Convert::ToString(System::Convert::ToInt32(recover));
					while (System::Convert::ToInt32(sorted_appliance[i, 3])>0){
						id1 = id;
						sign = 0;
						cheap_space = matrix[n][k_cheap[id1]];

						for (int k = ((System::Convert::ToInt32(sorted_appliance[i, 4])*(time_slot / 24)) + id1); k < System::Convert::ToInt32(sorted_appliance[i, 5])*(time_slot / 24); k++){
							if (cheap_space.kwh >= matrix[n][k].kwh && matrix[n][k].kwh!=0){
								cheap_space = matrix[n][k];
								sign = 1;
							}
						}

						if (sign == 1){
							//search similar with different k_cheap						
							for (int k = ((System::Convert::ToInt32(sorted_appliance[i, 4])*(time_slot / 24)) + id1); k < System::Convert::ToInt32(sorted_appliance[i, 5])*(time_slot / 24); k++){
								if (cheap_space.kwh == matrix[n][k].kwh && matrix[n][k].kwh != 0){
									k_cheap[id] = k;
									id++;
							...
							//k_cheap as key for inserting into matrix
							int id2;
							for (id2 = id1; id2 <= id; id2++){
								for (int j = 1; j < 7; j++){
									msclr::interop::marshal_context context;
									matrix[n][k_cheap[id2]].smart_grid[i][j] = context.marshal_as<std::string>(sorted_appliance[i, j]);
								}
								matrix[n][k_cheap[id2]].maxx = (matrix[n][k_cheap[id2]].maxx - System::Convert::ToInt32(sorted_appliance[i, 2]));
								matrix[n][k_cheap[id2]].smart_grid[i][7] = "1";
							}
						}
						sorted_appliance[i, 3] = System::Convert::ToString(System::Convert::ToInt32(sorted_appliance[i, 3]) - (id-id1+1));
					}
				}
				if ((System::Convert::ToInt32(sorted_appliance[i, 3])) <= 0){
					sorted_appliance[i, 7] = System::Convert::ToString(System::Convert::ToInt32(sorted_appliance[i, 7]) - 1);
					i++;
				}
			}

			// Display sel matrix mana aja yg isinya Kulkas
			for (int i = 1; i <= prog_count; i++) {
				for (int j = 0; j <= 48; j++) {
					if (matrix[i][j].smart_grid[1][1] == "Kewlkas")
						MessageBox::Show(System::Convert::ToString(i)+" "+System::Convert::ToString(j));
				}				
			}
